  ...
  stages {
    ...
    stage('Docker Build & Push Image') {
      steps {
        container('docker') {
            dir ("site") {
                sh ("docker build -t ${DOCKER_HUB_ACCOUNT}_${DOCKER_IMAGE_NAME}:${env.BUILD_NUMBER} .")
                sh ("docker push ${DOCKER_HUB_ACCOUNT}_${DOCKER_IMAGE_NAME}:${env.BUILD_NUMBER}")
                sh ("docker tag ${DOCKER_HUB_ACCOUNT}_${DOCKER_IMAGE_NAME}:${env.BUILD_NUMBER} ${DOCKER_HUB_ACCOUNT}_${DOCKER_IMAGE_NAME}:${JOB_NAME}-latest")
                sh ("docker push ${DOCKER_HUB_ACCOUNT}_${DOCKER_IMAGE_NAME}:latest")
            }
        }
      }
    }
